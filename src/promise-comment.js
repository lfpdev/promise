            ...
            if (typeof then === 'function') {
                ...
                then.call(x,
                    y => {
                        ...
                        // 当最终y不是promise,在【出口1或2】结束，或y返回失败，递归回到这里，嵌套的resolvePromise依次结束
...
class Promise {
    ...
    constructor(executor) {
        ...
        //   1.2 如果value非promise，则同步（调用）执行，将value赋值给THIS（如果放到定时器中，也是异步调用执行）
        // 2. THIS
        ...
        const resolve = (value) => {
            ...
            if (value instanceof Promise) {
                // 递归解析promise，直到value非promise或失败
                // 是异步执行（涉及到then）
                // 调用内部then方法，不会抛出异常
            ...
            if (((typeof value === 'object' && value !== null) || typeof value === 'function') &&
                ...
                // thenable 对象
                // 调用内部then方法，其回调是异步执行的，而调用thenable对象中then方法，其回调是同步的(调用then就会执行)
                // 因此这里需要在调用的时候异步（微任务）
            ...
            log.info(`------promise is------${this}`) // 打印resolve所属的promise
        }
        ...
        //   2.6 resolve 和 reject 的参数只能是值类型，如果是个表达式（new构造函数 或 普通函数调用），
    ...
    // 4. 错误处理
    //   4.1 如果距离自己最近的then没有传递第二个参数函数（默认参数函数直接将异常向下抛），则找下一个then或catch
    // 5. THIS
            ...
            if (this.status === RESOLVED) {
                ...
                process.nextTick(() => {
                    try {
                        ...
                        // 如果x是promise，调用该promise的then方法时，传递的两个参数函数是promise2的resolve reject
                        //   1. 如果该promise返回成功，则调用promise2的resolve方法，使当前then返回一个成功的promise2
                        //   2. 如果该promise返回失败，则调用promise2的reject方法，使当前then返回一个失败的promise2
                        resolvePromise(promise2, x, resolve, reject)
                    } catch (error) {
    ...
    // 调用callback不会传递参数（无法拿到前面promise的返回值）
        ...
        return this.then(value => {
            ...
            // 如果前面的promise报错，则进入这里，将前面promise的错误传递下去
            // 如果是自己（callback 执行）报错，不会进入then，直接传递下去
